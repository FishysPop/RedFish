...
const { Player, QueryType, useMainPlayer, useQueue } = require('discord-player');
      ...
      case "both":{
        try {
          ...
          if (Discordplayer) {
            ...
            Discordplayer.insertTrack(track, 0);
            client.totalTracksPlayed += 1;
            embed.setDescription(`**Playing Next: [${track.title}](${track.url}) - ${track.author}** \`${track.duration}\``);
                ...
                if (query) {
                  if (playerSettings.TidalNativePlay) {
                      ...
                      if (tidalSearchResult?.tracks?.length > 0) {
                          ...
                          try {
                              const nativeResult = await thirdPartySourceHandler.handleSource(track.uri, Lavaplayer, interaction.user, client, track, true, playerSettings);
                  ...
                  if (!res || !res.tracks.length) {
                      res = await Lavaplayer.search(query, { requester: interaction.user });
                  }
                }

                if (!res || !res.tracks.length) {
                  res = await Lavaplayer.search(name, { requester: interaction.user });
                }
              ...
              if (playerSettings.searchEngine === 'tidal' && playerSettings.TidalNativePlay) {
                  ...
                  if (tidalSearchResult?.tracks?.length > 0) {
                      ...
                      for (const engine of fallbackSearchStrategy) {
                          ...
                          res = await Lavaplayer.search(name, searchOptions);
                          if (res?.tracks.length) {
                              const track = res.tracks[0];
                              const isSpotifyResult = track.sourceName === 'spotify';
                              if (isSpotifyResult && playerSettings.SpotifyNativePlay) {
                                  const nativeResult = await thirdPartySourceHandler.handleSource(track.realUri, Lavaplayer, interaction.user, client, null, true, playerSettings);
                                  if (nativeResult) {
                                      res = nativeResult;
            ...
            return handlePlayError(interaction, name, e, Lavaplayer || null);
        }
      }
case "discord_player": {
    try {
